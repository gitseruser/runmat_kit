/**
 * Библиотека редактора — CodeMirror 6 (вынесена из исходного бандла index.js
 * при рефакторинге; см. CONTEXT.md → «Вынос CM6»). Содержит весь код CodeMirror,
 * необходимый интерфейсу. Экспорты, используемые в index.js/themes.js:
 *   Compartment, EditorState, EditorView, HighlightStyle, StreamLanguage,
 *   basicSetup, indentUnit, keymap, syntaxHighlighting, tags.
 * ВАЖНО: файл большой (~26 800 строк) — правки только точечные, по конкретным
 * экспортам; целиком не редактировать.
 */
let rangeFrom = [],
  rangeTo = [];
(() => {
  ...
  updateBlockWrappers() {
    ...
    for (let cur2 = this.blockWrappers; cur2.value && cur2.from <= this.pos; cur2.next())
      if (cur2.to >= this.pos) {
        ...
        let wrap = new OpenWrapper(cur2.from, cur2.to, cur2.value, rank),
...
class TileCache {
  ...
  find(cls, test, type = 2) {
    ...
    for (let j = 0; j < bucket.length; j++) {
      let index2 = (j + off) % bucket.length,
        tile = bucket[index2];
  ...
  getCompositionContext(text) {
    ...
    for (let parent = text.parentNode;; parent = parent.parentNode) {
      ...
      if (tile instanceof MarkTile)
        ...
        marks2.push(MarkTile.of(new MarkDecoration({
...
function moveVertically(view, start, forward, distance) {
  ...
  let halfText = view.viewState.heightOracle.textHeight >> 1,
    dist2 = distance !== null && distance !== void 0 ? distance : halfText;
...
class DOMChange {
  constructor(view, start, end, typeOver) {
    ...
    } = view.docView, curSel = view.state.selection;
  ...
  ensureHandlers(plugins) {
    ...
    for (let type in handlers2)
      if (type != "scroll") {
        ...
        if (!exists)
          dom.addEventListener(type, this.handleEvent, {
            passive
          });
      }
...
function basicMouseSelection(view, event) {
  ...
  return {
    ...
    get(event2, extend, multiple) {
      let cur2 = view.posAndSideAtCoords({
          x: event2.clientX,
          y: event2.clientY
        }, false),
        removed;
...
handlers.beforeinput = (view, event) => {
  ...
  if (event.inputType == "insertReplacementText" && view.observer.editContext) {
    let text = (_a2 = event.dataTransfer) === null || _a2 === void 0 ? void 0 : _a2.getData("text/plain"),
...
class ViewState {
  ...
  measure() {
    let {
      view
    } = this, dom = view.contentDOM, style = window.getComputedStyle(dom);
  ...
  viewportIsAppropriate({
    ...
    let {
      top: top2
    } = this.heightMap.lineAt(from, QueryType$1.ByPos, this.heightOracle, 0, 0);
    let {
      bottom
    } = this.heightMap.lineAt(to, QueryType$1.ByPos, this.heightOracle, 0, 0);
    let {
      visibleTop,
      visibleBottom
    } = this;
...
class EditContextManager {
  constructor(view) {
    ...
    this.handlers.textupdate = (e) => {
      let main2 = view.state.selection.main,
        {
          anchor,
          head
        } = main2;
  ...
  */
  createDeco(view) {
    ...
    for (let {
      ...
      of matchRanges(view, this.maxLength))
...
class Modifier {
  ...
  static get(base2, mods) {
    ...
    if (exists)
      return exists;
    let set = [],
      tag = new Tag(base2.name, set, base2, mods);
    for (let m of mods)
...
function insertCompletionText(state, text, from, to) {
  let {
    main: main2
  } = state.selection, fromOff = from - main2.from, toOff = to - main2.from;
  return {
    ...state.changeByRange((range) => {
      ...
      return {
        changes: {
          ...
          to: to == main2.from ? range.to : range.from + toOff,
          insert: lines
        },
...
function renderDiagnostic(view, diagnostic, inPanel) {
  ...
  return crelt("li", {
  ...
  }, diagnostic.renderMessage ? diagnostic.renderMessage(view) : diagnostic.message), (_a2 = diagnostic.actions) === null || _a2 === void 0 ? void 0 : _a2.map((action, i) => {
    let fired = false,
      click = (e) => {
        e.preventDefault();
        if (fired)
    ...
    let {
      name: name2
    } = action, keyIndex = keys2[i] ? name2.indexOf(keys2[i]) : -1;
  ...
  constructor(view) {
    ...
    let onkeydown = (event) => {
      ...
      if (event.keyCode == 27) {
        ...
        let {
          diagnostic
        } = this.items[this.selectedIndex], keys2 = assignKeys(diagnostic.actions);
  ...
  update() {
    ...
    diagnostics.between(0, this.view.state.doc.length, (_start, _end, {
      ...
      for (let diagnostic of spec.diagnostics) {
        if (seen2.has(diagnostic))
          continue;
        seen2.add(diagnostic);
        let found = -1,
          item;
...
/* CodeMirror 6 runtime - manually split from index.js (was lines 241-27018) */
export {
  Compartment,
  Decoration,
  EditorState,
